...
{
    public class PlcModbusMaster : IPlcModbus
    {
        public PlcModbusMaster()
        {
            this.connectionStateChangedEventManager = new ConnectionStateChangedEventManager(nameof(PlcModbusMaster));
        }

        public PlcModbusMaster(TcpClientComPortConfigInfo tcpClientComPortConfigInfo) :
            this()
        {
            this.tcpClientComPortConfigInfo = tcpClientComPortConfigInfo;
        }

        public PlcModbusMaster(SerialPortComPortConfigInfo serialPortComPortConfigInfo, PlcModbusTransportMode  plcModbusTransportMode = PlcModbusTransportMode.Rtu)
        {
        ...
        public void Initialize()
        {
            try
            {
                ...
                if (serialPortComPortConfigInfo != null)
                {
                    ...
                    if (plcModbusTransportMode == PlcModbusTransportMode.Rtu)
                    {
                        master = ModbusSerialMaster.CreateRtu(port);
                    }

                    if (plcModbusTransportMode == PlcModbusTransportMode.Ascii)
                    {
        ...
        private List<byte> GetBytes(PlcAddress address, bool isRam = false)
        {
        ...
        private readonly PlcModbusTransportMode plcModbusTransportMode;
    }
}
